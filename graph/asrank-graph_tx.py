...
avgLongitude = 0.0
for asn in asns:
    if asn["asn"] not in ["132203", "45090", "137876", "132591", "133478"]:
        continue
...
for asn in asns:
    ...
    for link in mpProvider[asn["asn"]]:
        # draw link edge:
        if mpAsns[link]["asn"] in ["132203", "45090", "137876", "132591", "133478"]:
        ...
        # add external AS
        if mpAsns[link]["asn"] not in ["132203", "45090", "137876", "132591", "133478"]:
            th1 = np.deg2rad(asn["longitude"])
            th2 = np.deg2rad(mpAsns[link]["longitude"])
            r1 = 1.0 - np.log10(asn["cone"]["numberAsns"]/maxClientConeSize)
            r2 = 1.0 - np.log10(mpAsns[link]["cone"]["numberAsns"]/maxClientConeSize)
            ax.plot(np.array([th1, th2]), np.array([r1, r2]), color="green", linewidth=1.0)
...
for asn in asns:
    ...
    for link in mpPeer[asn["asn"]]:
        ...
        if mpAsns[link]["asn"] not in ["132203", "45090", "137876", "132591", "133478"]:
            ...
            r1 = 1.0 - np.log10(asn["cone"]["numberAsns"]/maxClientConeSize)
            r2 = 1.0 - np.log10(mpAsns[link]["cone"]["numberAsns"]/maxClientConeSize)
            ax.plot(np.array([th1, th2]), np.array([r1, r2]), color="green", linewidth=1.0, alpha=0.5)
...
for asn in asns:
    if asn["country"]["iso"] not in ["132203", "45090", "137876", "132591", "133478"]:
            continue
    # select link set
    if asn["asn"] not in mpCustomer:
        continue
    for link in mpCustomer[asn["asn"]]:
        # draw link edge:
        if mpAsns[link]["asn"] in ["132203", "45090", "137876", "132591", "133478"]:
            th1 = np.deg2rad(asn["longitude"])
            th2 = np.deg2rad(mpAsns[link]["longitude"])
            r1 = 1.0 - np.log10(asn["cone"]["numberAsns"]/maxClientConeSize)
            r2 = 1.0 - np.log10(mpAsns[link]["cone"]["numberAsns"]/maxClientConeSize)
            ax.plot(np.array([th1, th2]), np.array([r1, r2]), color="gold", linewidth=1.0)

        print(mpAsns[link]["asn"], mpAsns[link]["asnName"], mpAsns[link]["country"]["iso"], mpAsns[link]["asnName"],
              mpAsns[link]["organization"]["orgName"], sep="\t")
        ...
        if mpAsns[link]["asn"] not in ["132203", "45090", "137876", "132591", "133478"]:
            ...
            r1 = 1.0 - np.log10(asn["cone"]["numberAsns"]/maxClientConeSize)
            r2 = 1.0 - np.log10(mpAsns[link]["cone"]["numberAsns"]/maxClientConeSize)
            ax.plot(np.array([th1, th2]), np.array([r1, r2]), color="gold", linewidth=1.0, alpha=0.5)

            theta_ex = np.append(theta_ex, np.deg2rad(mpAsns[link]["longitude"]))
            r_ex = np.append(r_ex, 1.0 - np.log10(mpAsns[link]["cone"]["numberAsns"] / maxClientConeSize))

            ax.plot(np.deg2rad(mpAsns[link]["longitude"]), 1.0 - np.log10(mpAsns[link]["cone"]["numberAsns"] / maxClientConeSize), '.',
                    markersize=10.0 - 1.0 + 1.0 * np.log10(mpAsns[link]["cone"]["numberAsns"] / maxClientConeSize), color='blue')
